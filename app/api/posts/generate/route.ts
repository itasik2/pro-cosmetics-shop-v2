...
export async function POST(req: Request) {
  try {
    ...
    /**
     * ВАЖНО:
     * У вас рендер статьи строит план по заголовкам в формате **...**
     * Поэтому генератор должен выдавать заголовки только так.
     * Никаких ## ### --- чтобы не “ломать” форматирование.
     */

    const slidesRule = blocks.slides
      ? `В каждом разделе добавляй подблок **Слайд-выжимка** (3–5 коротких буллетов, 1 строка = 1 буллет), чтобы текст легко превращался в презентацию.`
      : `Блок **Слайд-выжимка** не добавляй.`;

    const tableRule = blocks.table
      ? `Добавь ОДНУ Markdown-таблицу (строки с |) в отдельном блоке **Таблица сравнения**: сравни 3–5 вариантов (например: типы кожи/состояния/активы/сценарии применения/ошибки выбора).`
      : `Таблицу не добавляй.`;
    ...
    // 5) Промпт: просим ВЕРНУТЬ JSON и ничего кроме JSON
    const prompt = `
...
ФОРМАТИРОВАНИЕ (КРИТИЧНО ДЛЯ САЙТА):
...
СТРУКТУРА content:
1) **Лид**
   2–3 предложения, зачем читать и кому полезно.
2) **План**
   6–10 пунктов списком "- ...", каждый пункт — будущий раздел статьи.
3) Далее 6–9 секций, каждая секция оформляется так:
   **Название секции**
   - 3–6 тезисов списком
...
7) ${faqRule}
8) **Вывод**
   Кратко подведи итог (3–5 предложений) и добавь фразу:
   "Если остались вопросы — задайте их в Q&A на нашем сайте."

Содержательные требования:
- Практика важнее воды: больше критериев выбора, действий, предупреждений, комбинаций средств.
