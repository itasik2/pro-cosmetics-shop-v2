...
type Body = {
  ...
  blocks?: {
    slides?: boolean; // <-- ОСТАВЛЯЕМ ВОЗМОЖНОСТЬ, НО ПО ДЕФОЛТУ ВЫКЛ
    faq?: boolean;
    checklist?: boolean;
    mistakes?: boolean;
    table?: boolean;
...
export async function POST(req: Request) {
  try {
    ...
    const blocks = {
      // ВАЖНО: slides по умолчанию ВЫКЛЮЧЕНЫ
      slides: boolOrDefault(json.blocks?.slides, false),

      // остальное оставляем “насыщенным” по умолчанию
    ...
    // Если позже захотите включать “Слайд-выжимку” — просто передайте blocks.slides=true
    const slidesRule = blocks.slides
      ? `После каждого крупного раздела добавляй блок "**Слайд-выжимка**" (3–5 коротких буллетов).`
      : `Блок "**Слайд-выжимка**" НЕ добавляй.`;

    // 5) Промпт: под ваш рендер (жирные заголовки, без ##/###/---, без строки "Лид")
    const prompt = `
...
- В начале: 2–3 предложения вступления (без отдельной строки "Лид").
- Далее сделай секцию **План** и перечисли пункты списком "- ...".
- Затем 6–9 секций (каждая начинается с **...**), в каждой секции:
  - 3–6 тезисов списком "- ..."
  - 1–2 абзаца пояснения
  - мини-блок **Практика** (3–6 шагов/советов)
  - ${slidesRule}
...
- ${checklistRule}
- ${faqRule}
- Заверши секцией **Вывод** (кратко + предложение задать вопрос в Q&A на сайте)
